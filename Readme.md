**SV Event Queue**
  The SystemVerilog event queue refers to the simulator’s scheduling mechanism, organizing events.
  It organizes events into regions within a time step, ensuring deterministic execution and preventing race conditions.
...
**UVCs**
  ...
  Scalability: Multiple UVCs verify complex DUTs (e.g., PISO + SPI).
...
**UVM Phases**
  They provide a structured execution flow for testbenches, ensuring components are initialized, connected, and run in a consistent order.
  ...
  Run Phases (parallel):
    ...
    Sub-phases (time-consuming, optional, useful for complex protocols): reset_phase, configure_phase, main_phase, shutdown_phase.
...
**Objections**
  Objections in UVM control the duration of time-consuming phases (e.g., run_phase),
  ensuring simulation runs until all verification tasks complete. (e.g., raise_objection in piso_test).
  ...
  Phase Control: Without objections, run_phase ends immediately. Multiple components can raise objections.
  Granularity: Use in main_phase for finer control (not needed for PISO).
  Debugging: uvm_objection::trace() logs objection activity.
...
**TLM Components**
  Transaction-Level Modeling (TLM) components in UVM enable communication between testbench components (e.g., monitor to scoreboard)
  using high-level transactions instead of pin-level signals, improving modularity and scalability.

  Port: Initiates communication by sending transactions.
    Example: uvm_analysis_port for broadcasting.
  Imp (Implementation): Receives transactions, implementing methods like write.
    Example: uvm_analysis_imp in scoreboards.
  Export: Connects ports to imps, acting as a bridge.
  ...
  Decoupling: TLM ports allow components to communicate without direct references, enhancing reusability.
  Non-Blocking: write in analysis ports ensures no stalls, critical for monitor performance.
  Fifo: Useful for buffering in complex testbenches, not needed for simple PISO.
...
**TLM Methods**
  get, peek, put (and try variations) methods facilitate transaction passing between UVM components,
  typically used with uvm_tlm_fifo, uvm_get_port, or uvm_put_port, enabling blocking and non-blocking communication.
...
**UVM Analysis Port**
  The uvm_analysis_port broadcasts transactions to multiple subscribers (e.g., scoreboards, coverage collectors),
  supporting one-to-many communication in verification, without knowing their identities, decoupling the monitor from other components.
  ...
  Blocking Ports: uvm_get_port, uvm_put_port for driver-sequencer (not analysis).
  Hierarchical Connections: Via uvm_component hierarchy.
  Broadcast: One port can connect to multiple imps, unlike sequencer-driver (one-to-one).
  Non-Blocking: write executes instantly, preventing monitor stalls.
  Fifo Use: uvm_tlm_analysis_fifo decouples timing if subscribers are slow, rare for PISO.
  Push Model: UVM favors push (write) for analysis to simplify monitors.
  Pull Model: uvm_get_port for driver-sequencer, not analysis.
...
**uvm_tlm_analysis_fifo**
  ...
  Decoupling: FIFO allows monitor to proceed without waiting for cov.
  Blocking Get: get waits for transactions, suitable for run_phase.
  Size: Default unlimited, configurable via new(size). Monitor’s write blocks if FIFO is full.
...
**UVM object creation**
  Constructors initialize UVM objects (uvm_object) or components (uvm_component). UVM provides two creation methods:
   ...
   Constructor initializes basic properties (name, parent).Avoids heavy logic (e.g., object creation) to ensure phase-based initialization.
...
**uvm_config_db**
  A UVM mechanism for sharing configuration data (e.g., parameters, interfaces) across the testbench hierarchy.
  ...
  Syntax(set): uvm_config_db#(T)::set(cntxt, inst_name, field_name, value)
  ...
  Syntax(get): uvm_config_db#(T)::get(cntxt, inst_name, field_name, value)
  ...
  if set method is placed in tb initial block, uvm_config_db#(int)::set(null, "*", "num_txns", 20); -> null, "*" in tb is coarse, affecting all sequences.
    Suitable for global defaults (e.g., simulation-wide settings). -> Less flexible; all tests use the same num_txns.
  if set method is placed in test, uvm_config_db#(int)::set(this, "env.agt.seqr.seq", "num_txns", 20); -> "env.agt.seqr.seq" in test targets a specific sequence or wildcard.
    Allows test-specific configuration (e.g., 20 for one test, 50 for another -> More granular control, better for regressions.
  ...
  How do you debug a config_db failure?
    I check the scope string for typos, verify the set/get types match, and log the get result to confirm the value was retrieved.
...
**Coverage in UVM**
  ...
  Components:
    Covergroup: Defines a set of coverage points.
    Coverpoint: Tracks values of a variable (e.g., data_in).
    Bins: Subdivide values (e.g., 0, 15, [1:14]).
    ...
    Cross: Combines multiple coverpoints (e.g., data_in x load).
    ...
    Options: Control behavior (e.g., weight, goal).
  ...
  Typically implemented in monitors or subscribers to sample DUT transactions. Use uvm_subscriber for dedicated coverage collection.
...
**UVM Subscriber**
  It is a UVM component that receives transactions via an analysis port, typically for coverage collection or passive analysis.
...
**UVM Sequencer**
  ...
  Layered Sequencers: For complex protocols, stack sequencers (e.g., transaction to packet).
  ...
  start_item/finish_item:
    ...
    Sequence Operation:
      ...
      Steps:
        Sequence’s body task creates a req (e.g., piso_seq_item).
        start_item(req) gains sequencer access and randomizes req.
        finish_item(req) sends req to the driver, waits for completion.
        Driver applies req fields (e.g., req.load, req.data_in) to the DUT.
        Sequence repeats or ends, dropping objections if needed.
  ...
  Others:
    uvm_do_pri: Sets sequence priority. eg. ``uvm_do_pri(req, 100)`
    uvm_do_on: Specifies sequencer. eg. ``uvm_do_on(req, p_sequencer)`
    uvm_do_pri_with: Combines priority and constraints. eg.``uvm_do_pri_with(req, 100, { load == 1; data_in == 4'b0101; })`

  Convenience:
    Macros reduce boilerplate, but explicit start_item offers more control.
  Constraints:
    uvm_do_with overrides default randomization, critical for directed PISO tests.
  Arbitration:
    Priority in uvm_do_pri resolves sequence conflicts.
  ...
  What’s p_sequencer used for?
    p_sequencer is a typed handle to a sequencer, like piso_sequencer, letting a sequence access custom fields,
    unlike the generic m_sequencer. For PISO, I’d use it to toggle debug modes, but the default sequencer suffices for simple stimulus.
...
**UVM Factory Type Overrides**
  ...
  Types:
    Type Override:
      ...
      Syntax: base_class::type_id::set_type_override(derived_class::get_type());
        deprecated: uvm_factory::set_type_override_by_type(base_class::get_type(), derived_class::get_type());
    Instance Override:
      ...
      Syntax: base_class::type_id::set_inst_override(derived_class::get_type(), "path");
        deprecated: uvm_factory::set_inst_override_by_type(base_class::get_type(), derived_class::get_type(), "path");
  ...
  Priority: Instance overrides take precedence over type overrides.
...
**Dist operator in SystemVerilog**
  ...
  :=
    ...
    eg. load=1 gets ~30 occurrences, load=0 gets ~70 per 100 tries.
  ...
  :/
    ...
    eg. load=1 gets ~30% (30/100), load=0 gets ~70%.
